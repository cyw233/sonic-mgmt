...
def test_bgp_facts_ipv6_only(duthosts_ipv6_mgmt_only):
...
def test_show_features_ipv6_only(duthosts_ipv6_mgmt_only):
    # Add a temporary debug log to see if DUTs are reachable via IPv6 mgmt-ip. Will remove later
    log_eth0_interface_info(duthosts_ipv6_mgmt_only)
    with SafeThreadPoolExecutor(max_workers=8) as executor:
...
def test_image_download_ipv6_only(creds, duthosts_ipv6_mgmt_only):
    """
    ...
    def verify_image_download_ipv6_only(dut):
        ...
        for mgmt_interface in mgmt_interfaces:
            output = dut.shell("curl --fail --interface {} {}".format(mgmt_interface, image_url),
...
def test_snmp_ipv6_only(request, duthosts_ipv6_mgmt_only, localhost, creds_all_duts):
    # Add a temporary debug log to see if DUTs are reachable via IPv6 mgmt-ip. Will remove later
    log_eth0_interface_info(duthosts_ipv6_mgmt_only)
...
def test_ro_user_ipv6_only(request, localhost, ptfhost, duthosts_ipv6_mgmt_only, tacacs_creds):
    # Add a temporary debug log to see if DUTs are reachable via IPv6 mgmt-ip. Will remove later
    log_eth0_interface_info(duthosts_ipv6_mgmt_only)
...
def test_rw_user_ipv6_only(request, localhost, ptfhost, duthosts_ipv6_mgmt_only, tacacs_creds):
    # Add a temporary debug log to see if DUTs are reachable via IPv6 mgmt-ip. Will remove later
    log_eth0_interface_info(duthosts_ipv6_mgmt_only)
...
def test_telemetry_output_ipv6_only(request, duthosts_ipv6_mgmt_only, localhost, ptfhost, gnxi_path):
    # Add a temporary debug log to see if DUTs are reachable via IPv6 mgmt-ip. Will remove later
    log_eth0_interface_info(duthosts_ipv6_mgmt_only)
